...
import asyncio, aiohttp, os, uuid, subprocess
...
async def generate_video(req: VideoRequest, background_tasks: BackgroundTasks):
  ...
  async def process_job(job_id: str, req: VideoRequest):
    ...
    try:
        ...
        audio = await generate_voice(script, req.language, tmp_dir)
...
async def generate_voice(script, language, tmp_dir):
...
def render_video(clips, audio, output, duration):
    ...
    subprocess.run(["ffmpeg","-y","-i",concat,"-i",audio,"-c:v","copy","-c:a","aac","-shortest",output], check=True, capture_output=True)
...
async def upload_video(path, job_id):
    async with aiohttp.ClientSession() as s:
        with open(path, "rb") as f:
            ...
            r = await s.post(f"{SUPABASE_URL}/storage/v1/object/videos/{job_id}.mp4", headers={"Authorization": f"Bearer {SUPABASE_KEY}", "x-upsert": "true"}, data=data)
